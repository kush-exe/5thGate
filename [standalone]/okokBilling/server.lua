...
QBCore.Functions.CreateCallback("okokBilling:GetInvoices", function(source, cb)
	...
	MySQL.query('SELECT * FROM okokBilling WHERE receiver_identifier = @identifier ORDER BY CASE WHEN status = "unpaid" THEN 1 WHEN status = "autopaid" THEN 2 WHEN status = "paid" THEN 3 WHEN status = "cancelled" THEN 4 END ASC, id DESC', {
...
QBCore.Functions.CreateCallback("okokBilling:GetSocietyInvoices", function(source, cb, society)
	...
	MySQL.query('SELECT * FROM okokBilling WHERE society_name = @society ORDER BY id DESC', {
		['@society'] = society
	}, function(result)
		local invoices = {}
...
function checkTimeLeft()
	MySQL.query('SELECT *, TIMESTAMPDIFF(HOUR, limit_pay_date, CURRENT_TIMESTAMP()) AS "timeLeft" FROM okokBilling WHERE status = @status', {
		['@status'] = 'unpaid'
	}, function(result)
		if result ~= nil then
			for k, v in ipairs(result) do
					...
					for k, vl in pairs(whenToAddFees) do
						if v.fees_amount == k - 1 then
							if v.timeLeft >= vl*(-1) then
								MySQL.query('UPDATE okokBilling SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
									['@fees_amount'] = k,
				...
				elseif v.timeLeft >= 0 and Config.PayAutomaticallyAfterLimit then
					...
					if xPlayer == nil then
						MySQL.query('SELECT money FROM players WHERE citizenid = @id', {
							...
							if account[1] ~= nil then
								...
								MySQL.query('UPDATE players SET money = @playerAccount WHERE citizenid = @target', {
									...
									MySQL.query('UPDATE okokBilling SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
										['@paid'] = 'autopaid',
										['@id'] = v.id
									})
								end)
	...
	SetTimeout(1 * 60000, checkTimeLeft)
end
