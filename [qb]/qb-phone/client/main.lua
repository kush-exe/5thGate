...
RegisterCommand('+phoneopen', function()
    if not PhoneData.isOpen then
        local IsHandcuffed = false
        if not IsHandcuffed then
            if not IsEntityPlayingAnim(PlayerPedId(), "dead", "dead_a", 3) and not IsEntityPlayingAnim(PlayerPedId(), "combat@damage@writhe", "writhe_loop", 3) then
                OpenPhone()
...
local takePhoto = false
    RegisterNUICallback('PostNewImage', function(data, cb)
        ...
        exports['screenshot-basic']:requestScreenshotUpload('https://ptb.discord.com/api/webhooks/798684601803079690/EjITuZiUWMOPR1V5HJhVylv5ZwvfljFwNmX395JzHfVAY4E4e1fMi86sslSODEm5-l-l', 'files[]', function(data2)
            DestroyMobilePhone()
...
AddEventHandler('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    PhoneData.PlayerData.money.bank = newmoney&dollar
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "SCBanking", text = "There is &dollar;"..amount.." credited!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "SCBanking", content = "There is &dollar;"..amount.." credited!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        exports['pma-voice']:addPlayerToCall(PhoneData.CallData.CallId, 'Phone')
    else
        PhoneData.CallData.InCall = false
        PhoneData.CallData.CallType = nil
        PhoneData.CallData.AnsweredCall = false
