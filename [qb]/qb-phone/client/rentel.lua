...
function SpawnVehicle(data)
    TriggerServerEvent('qb-phone:server:spawnVehicle', data)
    Wait(1500)
end

RegisterNetEvent('qb-phone:client:spawnVehicle')
AddEventHandler('qb-phone:client:spawnVehicle', function(data)
    CloseMenu()
        
    if Config.RentelVehicles[data.model]['stored'] then
        Config.RentelVehicles[data.model]['stored'] = false
    else
        TriggerServerEvent('qb-phone:server:removeMoney', 'cash', data.price)
    end
...
function RestoreVehicle(model)
    ...
    if Config.RentelVehicles[model] and Config.RentelVehicles[model]['stored'] then
        ...
        TriggerServerEvent('qb-phone:server:restoreRented', math.floor(Config.RentelVehicles[model]['price'] / 2))
    end
end
...
function RentVehicle(model)
    ...
    if Config.RentelVehicles[model] and not Config.RentelVehicles[model]['stored'] and location and Config.RentelVehicles[model].price <= QBCore.Functions.GetPlayerData()['money']['bank'] then
        TriggerServerEvent('qb-phone:server:removeMoney', 'bank', Config.RentelVehicles[model]['price'])
