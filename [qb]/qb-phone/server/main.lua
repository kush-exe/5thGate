...
QBCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    ...
    if Player ~= nil then
        ...
        exports.oxmysql:execute("SELECT * FROM player_contacts WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ORDER BY `name` ASC", function(result)
            local Contacts = {}
           ...
           exports.oxmysql:execute("SELECT * FROM phone_invoices WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(invoices)
               ...
               exports.oxmysql:execute("SELECT * FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(garageresult)
                   ...
                   exports.oxmysql:execute("SELECT * FROM phone_messages WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(messages)
                       ...
                       exports.oxmysql:execute('SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(mails)
                           ...
                           exports.oxmysql:execute('SELECT * FROM `crypto_transactions` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(transactions)
...
AddEventHandler('qb-phone:server:RemoveMail', function(MailId)
   ...
   exports.oxmysql:execute('DELETE FROM `player_mails` WHERE `mailid` = "'..MailId..'" AND `citizenid` = "'..Player.PlayerData.citizenid..'"')
...
AddEventHandler('qb-phone:server:sendNewMail', function(mailData, srcz)
    local src = source
    ...
    SetTimeout(200, function()
       exports.oxmysql:execute('SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` DESC', function(mails)
            if mails[1] ~= nil then
                for k, v in pairs(mails) do
                    if mails[k].button ~= nil then
                        mails[k].button = json.decode(mails[k].button)
...
AddEventHandler('qb-phone:server:ClearButtonData', function(mailId)
   ...
   exports.oxmysql:execute('UPDATE `player_mails` SET `button` = "" WHERE `mailid` = "'..mailId..'" AND `citizenid` = "'..Player.PlayerData.citizenid..'"')
    SetTimeout(200, function()
       exports.oxmysql:execute('SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` DESC', function(mails)
            if mails[1] ~= nil then
                for k, v in pairs(mails) do
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                SCPhone.SetPhoneAlerts(Player.PlayerData.citizenid, "twitter")
                SCPhone.AddMentionedTweet(Player.PlayerData.citizenid, TweetMessage)
               ...
               exports.oxmysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE '%"..firstName.."%' AND `charinfo` LIKE '%"..lastName.."%'", function(result)
                    if result[1] ~= nil then
                        local MentionedTarget = result[1].citizenid
                        SCPhone.SetPhoneAlerts(MentionedTarget, "twitter")
                        SCPhone.AddMentionedTweet(MentionedTarget, TweetMessage)
                    end
                end)
            end
        end
...
AddEventHandler('qb-phone:server:SetPhoneAlerts', function(app, alerts)
    ...
    SCPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)
   ...
   exports.oxmysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
               ...
               exports.oxmysql:execute("UPDATE `players` SET `money` = '"..json.encode(moneyInfo).."' WHERE `citizenid` = '"..result[1].citizenid.."'")
...
AddEventHandler('qb-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
   exports.oxmysql:execute("UPDATE `player_contacts` SET `name` = '"..newName.."', `number` = '"..newNumber.."', `iban` = '"..newIban.."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `name` = '"..oldName.."' AND `number` = '"..oldNumber.."'")
end)
...
AddEventHandler('qb-phone:server:RemoveContact', function(Name, Number)
   ...
   exports.oxmysql:execute("DELETE FROM `player_contacts` WHERE `name` = '"..Name.."' AND `number` = '"..Number.."' AND `citizenid` = '"..Player.PlayerData.citizenid.."'")
end)
...
AddEventHandler('qb-phone:server:AddNewContact', function(name, number, iban)
   ...
   exports.oxmysql:execute("INSERT INTO `player_contacts` (`citizenid`, `name`, `number`, `iban`) VALUES ('"..Player.PlayerData.citizenid.."', '"..tostring(name).."', '"..tostring(number).."', '"..tostring(iban).."')")
end)
...
AddEventHandler('qb-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
   ...
   exports.oxmysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE '%"..ChatNumber.."%'", function(Player)
        if Player[1] ~= nil then
            local TargetData = QBCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)

            if TargetData ~= nil then
               exports.oxmysql:execute("SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        -- Update for target
                       exports.oxmysql:execute("UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..TargetData.PlayerData.citizenid.."' AND `number` = '"..SenderData.PlayerData.charinfo.phone.."'")
                                
                        -- Update for sender
                       exports.oxmysql:execute("UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..TargetData.PlayerData.charinfo.phone.."'")
                        ...
                        -- Insert for target
                       exports.oxmysql:execute("INSERT INTO `phone_messages` (`citizenid`, `number`, `messages`) VALUES ('"..TargetData.PlayerData.citizenid.."', '"..SenderData.PlayerData.charinfo.phone.."', '"..json.encode(ChatMessages).."')")
                                            
                        -- Insert for sender
                       exports.oxmysql:execute("INSERT INTO `phone_messages` (`citizenid`, `number`, `messages`) VALUES ('"..SenderData.PlayerData.citizenid.."', '"..TargetData.PlayerData.charinfo.phone.."', '"..json.encode(ChatMessages).."')")
               ...
               exports.oxmysql:execute("SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        -- Update for target
                       exports.oxmysql:execute("UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..Player[1].citizenid.."' AND `number` = '"..SenderData.PlayerData.charinfo.phone.."'")
                                
                        -- Update for sender
                        Player[1].charinfo = json.decode(Player[1].charinfo)
                       exports.oxmysql:execute("UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..Player[1].charinfo.phone.."'")
                    else
                        -- Insert for target
...
AddEventHandler('qb-phone:server:SaveMetaData', function(MData)
   ...
   exports.oxmysql:execute("SELECT * FROM `players` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
       ...
       exports.oxmysql:execute("UPDATE `players` SET `metadata` = '"..json.encode(MetaData).."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")
    end)
...
QBCore.Functions.CreateCallback('qb-phone:server:FetchResult', function(source, cb, search)
   ...
   exports.oxmysql:execute(query, function(result)
       exports.oxmysql:execute('SELECT * FROM `apartments`', function(ApartmentData)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetVehicleSearchResults', function(source, cb, search)
    ...
    local search = escape_sqli(search)
    local searchData = {}
   exports.oxmysql:execute('SELECT * FROM `player_vehicles` WHERE `plate` LIKE "%'..search..'%" OR `citizenid` = "'..search..'"', function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
...
QBCore.Functions.CreateCallback('qb-phone:server:GetGarageVehicles', function(source, cb)
   ...
   exports.oxmysql:execute("SELECT * FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                local VehicleData = QBCore.Shared.Vehicles[v.vehicle]

                local VehicleGarage = "Not in garage"
...
AddEventHandler('qb-phone:server:AddTransaction', function(data)
   ...
   exports.oxmysql:execute("INSERT INTO `crypto_transactions` (`citizenid`, `title`, `message`) VALUES ('"..Player.PlayerData.citizenid.."', '"..escape_sqli(data.TransactionTitle).."', '"..escape_sqli(data.TransactionMessage).."')")
end)
...
RegisterServerEvent('qb-phone:server:restoreRented')
AddEventHandler('qb-phone:server:restoreRented', function(money)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-phone:server:removeMoney')
AddEventHandler('qb-phone:server:removeMoney', function(type, money)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
AddEventHandler('qb-phone:server:clearVehicleTrunk', function(plate)
   exports.oxmysql:execute("DELETE FROM `trunkitems` WHERE `plate` = '"..plate.."'")
end)