                ...
                for k, v in pairs(result) do
                    ...
                    elseif result["WEAPONWOUNDS"] then
                        for _, v2 in pairs(result["WEAPONWOUNDS"]) do
                            QBCore.Functions.Notify('Status: '.. QBCore.Shared.Weapons[v2].damagereason)
                        end
                    elseif result["BLEED"] > 0 then
                        QBCore.Functions.Notify('Status: '.. Config.BleedingStates[v].label)
                    else
                ...
                local ped = PlayerPedId()
                    if IsPedInAnyVehicle(ped, false) then
                        ...
                        QBCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                            ...
                            exports['ps-fuel']:SetFuel(veh, 100.0)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh), Config.Helicopter)
                            --SetVehicleEngineOn(veh, true, true)
                        end, coords, true)
                    end
                end
