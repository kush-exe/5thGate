SetupTruckerInfo = function(data) {
    ...
    $("#trucker-name").html(JPR.Phone.Data.PlayerData.charinfo.firstname + " " + JPR.Phone.Data.PlayerData.charinfo.lastname);
    ...
    if (data.CurrentTierData.min == data.CurrentTierData.max) {
        ...
        $("#trucker-header-progress-next").html("Prox: MAX");
    
        $(".trucker-header-progress-fill").css("width", "100%");
    } else {
        $("#trucker-header-progress-current").html("Atual: " + data.CurrentRep + " REP");
        $("#trucker-header-tier").html("Tier " + data.CurrentTier);
        $("#trucker-header-progress-next").html("Prox: " + (data.CurrentTierData.max - data.CurrentRep) + " REP");
