...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(JPR.Phone.Data.PlayerData.citizenid, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        JPR.Phone.Notifications.Add("corrida.png", "Racing", "The race has already started..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            var Locked = '<i class="fas fa-unlock"></i> Not started';
            if (race.RaceData.Started) {
                Locked = '<i class="fas fa-lock"></i> Start';
            }
            var LapLabel = "";
            if (race.Laps == 0) {
                LapLabel = "P2P"
            ...
            var InRace = IsInRace(JPR.Phone.Data.PlayerData.citizenid, race.RaceData.Racers);
            var Creator = IsCreator(JPR.Phone.Data.PlayerData.citizenid, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('https://jpr-newphone/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('https://jpr-newphone/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('https://jpr-newphone/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('https://jpr-newphone/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
                            ...
                            JPR.Phone.Notifications.Add("corrida.png", "Racing", "You are in the creator..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            JPR.Phone.Notifications.Add("corrida.png", "Racing", "Are you already in a race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('https://jpr-newphone/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('https://jpr-newphone/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://jpr-newphone/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
        ...
        if (TrackData.Records.Holder !== undefined) {
            ...
            $(".racing-setup-information-wr").html('MV: ' + secondsTimeSpanToHMS(TrackData.Records.Time) + ' ('+Holder+')');
        } else {
            $(".racing-setup-information-wr").html('MV: N/A');
        }
    });
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('https://jpr-newphone/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('https://jpr-newphone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('https://jpr-newphone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        JPR.Phone.Notifications.Add("corrida.png", "Racing", "You are in a race..", "#1DA1F2");
                    }
                });
            } else {
                JPR.Phone.Notifications.Add("corrida.png", "Racing", "You are already configuring..", "#1DA1F2");
            }
        } else {
            JPR.Phone.Notifications.Add("corrida.png", "Racing", "You don't have permissions for this..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        if (TrackName == '') TrackName = 'Qual o nome?'
        $.post('https://jpr-newphone/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('https://jpr-newphone/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    JPR.Phone.Notifications.Add("corrida.png", "Racing", "Name not available..", "#1DA1F2");
                }
            } else {
                JPR.Phone.Notifications.Add("corrida.png", "Racing", "You don't have permissions..", "#1DA1F2");
            }
        });
    } else {
        JPR.Phone.Notifications.Add("corrida.png", "Racing", "You have to choose a name..", "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('https://jpr-newphone/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('https://jpr-newphone/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('https://jpr-newphone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('https://jpr-newphone/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    $(".racing-setup").animate({
                                        ...
                                        $(".racing-setup-information-distance").html('Selecionar pista');
                                        $(".racing-setup-information-creator").html('Selecionar pista');
                                        $(".racing-setup-information-wr").html('Selecionar pista');
                                        $(".racing-setup-laps").val("");
                                        $('.dropdown').find('input').removeAttr('value');
                                        $('.dropdown').find('span').text("Selecionar pista");
                                    });
                                } else {
                                    JPR.Phone.Notifications.Add("corrida.png", "Racing", "It can't be that ..", "#1DA1F2");
                                }
                            });
                        } else {
                            JPR.Phone.Notifications.Add("corrida.png", "Racing", "Enter the number of laps..", "#1DA1F2");
                        }
                    } else {
                        JPR.Phone.Notifications.Add("corrida.png", "Racing", "Choose the track..", "#1DA1F2");
                    }
                }
            })
        } else {
...
$(document).on('click', '#setup-race-cancel', function(e){
    ...
    $(".racing-setup").animate({
        ...
        $(".racing-setup-information-distance").html('Seleciona a pista');
        $(".racing-setup-information-creator").html('Seleciona a pista');
        $(".racing-setup-information-wr").html('Seleciona a pista');
        $(".racing-setup-laps").val("");
        $('.dropdown').find('input').removeAttr('value');
        $('.dropdown').find('span').text("Seleciona a pista");
    });
});
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('https://jpr-newphone/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
