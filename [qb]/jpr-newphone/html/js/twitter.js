...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://jpr-newphone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    JPR.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});
...
JPR.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = JPR.Phone.Functions.FormatTwitterMessage(clean);
...
$(document).on('click','#twt-delete-click',function(e){
    ...
    $.post('https://jpr-newphone/DeleteTweet', JSON.stringify({id: source}))
})
...
JPR.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    $('#tweet-new-url').val("");
    if (Tweets.length > 0) {
...
JPR.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '.image', function(e){
    ...
    JPR.Screen.popUp(source, "twitter")
});
...
var gifsabertos = false;
$(document).on('click', '#tweet-abrir-gifs', function(e){
    e.preventDefault();

    if (gifsabertos == false) {
        $(".gifs-fundo").css({"display":"block"});
        gifsabertos = true;
        emojisnowpp = false;
        emojisnotwt = true;
        emojisnoolx = false;
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://jpr-newphone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            JPR.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    JPR.Phone.Notifications.Add("twitter.png", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

JPR.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
        ...
        $.each(hashtags, function(i, hashtag){
            ...
            if (hashtag.messages.length >= MinimumTrending) {
                ...
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Recent Hastag</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            }
...
JPR.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    JPR.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
