...
SetupMeosHome = function() {
    $("#meos-app-name").html("Olá " + JPR.Phone.Data.PlayerData.charinfo.firstname + " " + JPR.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.person-search-result', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Nome: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">NIF: '+ClickedPersonData.citizenid+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">D. Nascimento: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-number">Nr. Tel.: '+ClickedPersonData.phone+'</div> <div class="person-search-result-nationality">Nacionalidade: '+ClickedPersonData.nationality+'</div> <div class="person-search-result-gender">Gênero: '+Gender+'</div> &nbsp; <div class="person-search-result-apartment"><span id="'+ClickedPersonId+'">Residência: '+appartementData.label+'</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="'+ClickedPersonId+'"></i></div> &nbsp; <div class="person-search-result-warned">Buscas: '+IsWarrant+'</div> <div class="person-search-result-driverslicense">Carta: '+HasLicense+'</div>';
...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('https://jpr-newphone/SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('https://jpr-newphone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    JPR.Phone.Notifications.Add("meos.png", "MDT", "Copy!", "#badc58", 1750);
...
$(document).on('click', '.person-search-result-house', function(e){
    ...
    var OpenElement = '<div class="person-search-result-name">Dono: '+ClickedHouseData.charinfo.firstname+' '+ClickedHouseData.charinfo.lastname+'</div><div class="person-search-result-bsn">Residência: '+ClickedHouseData.label+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Endreço: '+ClickedHouseData.label+' &nbsp; <i class="fas fa-map-marker-alt house-adress-location" id="'+ClickedHouseId+'"></i></div> <div class="person-search-result-number">Tier: '+ClickedHouseData.tier+'</div> <div class="person-search-result-nationality">Garage: ' + GarageLabel + '</div>';
    ...
    if (OpenedHouse === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Dono: '+PreviousPersonData.charinfo.firstname+' '+PreviousPersonData.charinfo.lastname+'</div> <div class="person-search-result-bsn">Residência: '+PreviousPersonData.label+'</div>';
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://jpr-newphone/FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                ...
                JPR.Phone.Notifications.Add("meos", "MDT", "No results!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://jpr-newphone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                $.each(result, function (i, house) {
                    var PersonElement = '<div class="person-search-result-house" id="personhouse-'+i+'"><div class="person-search-result-name">Dono: '+house.charinfo.firstname+' '+house.charinfo.lastname+'</div> <div class="person-search-result-bsn">Residência: '+house.label+'</div></div>';
                    $(".person-search-results").append(PersonElement);
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://jpr-newphone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
                $.each(result, function (i, vehicle) {
                    ...
                    var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Matricula: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Dono: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">MOT: '+APK+'</div> <div class="vehicle-search-result-warrant">Busca: '+Flagged+'</div> </div>'
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('https://jpr-newphone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Sim";
            if (!vehicle.status) {
            ...
            JPR.Phone.Notifications.Add("meos", "MDT", "No vehicles nearby!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            $.post('https://jpr-newphone/SetAlertWaypoint', JSON.stringify({
                alert: alertData,
            }));
        } else {
            JPR.Phone.Notifications.Add("meos", "MDT", "This alert does not have GPS!");
        }
    }else {
        JPR.Phone.Notifications.Add("meos", "MDT", "No alerts available.");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    $(".meos-alerts").html("");
    $(".meos-recent-alerts").html('<div class="meos-recent-alert"> <span class="meos-recent-alert-title">You have no alert!</span></div>');
    JPR.Phone.Notifications.Add("meos", "MDT", "All alerts cleared!");
});
