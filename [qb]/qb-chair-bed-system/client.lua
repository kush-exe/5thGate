...
Citizen.CreateThread(function()
    for _,v in pairs(Config.objects.locations) do
        if v.object ~= nil then
            hash = GetHashKey(v.object)
        else
            hash = v.hash
        end
        exports['qb-target']:AddTargetModel(hash, {
            options = {
                {
                    event = "ChairBedSystem:Client:sit",
                    icon = "fab fa-chair",
                    label = "Sit",
                },
            },
...
-- event for target
RegisterNetEvent('ChairBedSystem:Client:sit')
AddEventHandler('ChairBedSystem:Client:sit', function()
    oElement = {}
    plyCoords = GetEntityCoords(PlayerPedId())
    for _, element in pairs(Config.objects.locations) do
        local closestObject = nil
        if element.object ~= nil then
            closestObject = GetClosestObjectOfType(plyCoords.x, plyCoords.y, plyCoords.z, 3.0, GetHashKey(element.object), 0, 0, 0)
        else
            closestObject = GetClosestObjectOfType(plyCoords.x, plyCoords.y, plyCoords.z, 3.0, element.hash, 0, 0, 0)
        end
    ...
    --checks if in range and then triggers sit event
    if isWithinObject and oElement.fObject ~= 0 then
        ...
        if (distanceDiff < 2.0 and not InUse) then
            if (oElement.fObjectIsBed == true) then
                TriggerServerEvent('qb-chair-bed-system:Server:Enter', oElement, oElement.fObjectCoords)
            else    
                TriggerServerEvent('qb-chair-bed-system:Server:Enter', oElement, oElement.fObjectCoords)
            end     
        end
        ...
        CreateThread(function()
            while true do
                ...
                if IsControlJustPressed(0, Config.objects.ButtonToStandUp) then
                    InUse = false
                    TriggerServerEvent('qb-chair-bed-system:Server:Leave', oElement.fObjectCoords)
...
RegisterNetEvent('qb-chair-bed-system:Client:Animation', function(v, coords)
