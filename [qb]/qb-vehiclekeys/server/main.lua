...
QBCore.Functions.CreateUseableItem("electronickit", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent("lockpicks:UseEK", source)
end)

QBCore.Commands.Add("hackcar", "hack into a car", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent("vehiclekeys:server:removeek")
AddEventHandler("vehiclekeys:server:removeek", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
